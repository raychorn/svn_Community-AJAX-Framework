...
<cfscript>
	...
	URL.computerID = URLDecode(URL.computerID);
	URL.serverName = URLDecode(URL.serverName);
	URL.userName = URLDecode(URL.userName);

	if ( (Len(Trim(URL.userName)) gt 0) AND (ListLen(URL.userName, '@') eq 2) AND (ListLen(URL.userName, '.') gt 1) ) {
		...
		if (NOT Request.dbError) {
			if (Request.qIsUsernameInvalid.recordCount eq 0) {
							...
							if (NOT Request.dbError) {
								...
								if ( (NOT Request.dbError) AND (sid neq -1) ) {
									if (NOT bool_isServerNameTaken) {
										...
										if (NOT isError) {
											if ( (ListLen(URL.computerID, '|') eq 3) AND (Len(URL.serverName) gt 0) ) {
												...
												if (NOT Request.dbError) {
													...
													optionsStruct.bcc = 'raychorn@hotmail.com';
											...
											//		Request.commonCode.cf_removeDirectory(ExpandPath(aFilePath));
													if (NOT Request.isPKviolation) {
														...
														dataStream = 'Warning 102. Another Runtime License cannot be created at this time because there is already a Runtime License issued.';
													}
												}
											} else {
										...
										dataStream = 'Error 105. The Runtime License cannot be assigned for the server name provided because that server name is already assigned to another Runtime License for this product.';	
									}
								} else {
									extraError = extraError & Request.explainErrorText;
...
<cfif (FindNoCase('Error ', dataStream) gt 0)>
	<cfscript>
		Request.commonCode.safely_cfmail('raychorn@hotmail.com', 'do-not-respond@ez-ajax.com', 'ezAJAX(tm) Registration System Error Report', errorReportNotification(dataStream, extraError));
	</cfscript>
