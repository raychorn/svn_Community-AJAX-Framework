...
AJAXEngine.prototype = {
	...
	initXmlHttpReqObject : function() {
		_alert('initXmlHttpReqObject() :: (!!this.xmlHttpReqObject) = [' + (!!this.xmlHttpReqObject) + ']');
		if (!!this.xmlHttpReqObject) {
			this.xmlHttpReqObject.oRequest = false;
			this.xmlHttpReqObject.oGateway = -1;
		}
		return this.xmlHttpReqObject;
	},
	// +++
	processXmlHttpRequestStateChange : function() {   
		window.status = this.xmlHttpReqObject.oRequest.readyState + ', ' + this.xmlHttpReqObject.oRequest.status;
		_alert('processXmlHttpRequestStateChange()' + ' window.status = [' + window.status + ']');
		_alert('processXmlHttpRequestStateChange()' + ' (!!this.xmlHttpReqObject.oRequest) = [' + (!!this.xmlHttpReqObject.oRequest) + ']');
		_alert('processXmlHttpRequestStateChange()' + ' this.xmlHttpReqObject.oRequest.readyState = [' + this.xmlHttpReqObject.oRequest.readyState + ']');
	    if ( (!!this.xmlHttpReqObject.oRequest) && (this.xmlHttpReqObject.oRequest.readyState == 4) ) {
	        try {
				_alert('processXmlHttpRequestStateChange()' + ' this.xmlHttpReqObject.oRequest.status = [' + this.xmlHttpReqObject.oRequest.status + ']');
	            if (this.xmlHttpReqObject.oRequest.status && this.xmlHttpReqObject.oRequest.status == 200) {
					var response = this.xmlHttpReqObject.oRequest.responseText;
					...
					if (!!this.xmlHttpReqObject.oGateway) {
						this.xmlHttpReqObject.oGateway.receivePacket(response);
					}
	            } else {
					var response = this.xmlHttpReqObject.oRequest.responseText;
	...
	initXmlHttpRequest : function(url) {
		var bool = false;
		this.initXmlHttpReqObject();
		_alert('window.XMLHttpRequest = [' + window.XMLHttpRequest + ']' + ', window.ActiveXObject = [' + window.ActiveXObject + ']');
	    if (window.XMLHttpRequest) { 
	    	try {
				this.xmlHttpReqObject.oRequest = new XMLHttpRequest();
				bool = true;
	        } catch(e) {
				this.xmlHttpReqObject.oRequest = false;
				bool = false;
	       	...
	       	try {
	        	this.xmlHttpReqObject.oRequest = new ActiveXObject("Msxml2.XMLHTTP");
				bool = true;
	      	} catch(e) {
	        	try {
	          		this.xmlHttpReqObject.oRequest = new ActiveXObject("Microsoft.XMLHTTP");
					bool = true;
	        	} catch(e) {
	          		this.xmlHttpReqObject.oRequest = false;
					bool = false;
	...
	_alert('initXmlHttpRequest(' + url + ')' + '\nthis.xmlHttpReqObject.oRequest = [' + this.xmlHttpReqObject.oRequest + ']');
	_alert('this.method = [' + this.method + ']');
		try {
			switch (this.method) {
				case this.Post_symbol:
					_alert('POST :: (!!this.xmlHttpReqObject.oRequest) = [' + (!!this.xmlHttpReqObject.oRequest) + ']');
					if (!!this.xmlHttpReqObject.oRequest) {
						this.xmlHttpReqObject.oRequest.onreadystatechange = this.processXmlHttpRequestStateChange;
						_alert('POST :: this.xmlHttpReqObject.oRequest.onreadystatechange = [' + this.xmlHttpReqObject.oRequest.onreadystatechange + ']');
						this.xmlHttpReqObject.oGateway = this; 
						var a = url.split('?');
						if (a.length == 2) {
							this.xmlHttpReqObject.oRequest.open("POST", a[0], true);
							this.xmlHttpReqObject.oRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");			
			                this.xmlHttpReqObject.oRequest.send('QUERY_STRING=' + a[1].URLEncode());
						} else {
							bool = false;
						}
					}
				...
				case this.Get_symbol:
					_alert('GET :: (!!this.xmlHttpReqObject.oRequest) = [' + (!!this.xmlHttpReqObject.oRequest) + ']');
					if (!!this.xmlHttpReqObject.oRequest) {
						try {
							this.xmlHttpReqObject.oRequest.onreadystatechange = this.processXmlHttpRequestStateChange;
						} catch(e) {
							_alertError(jsErrorExplainer(e, 'A. (initXmlHttpRequest) :: initXmlHttpRequest threw an error.'));
						}
						_alert('GET :: this.xmlHttpReqObject.oRequest.onreadystatechange = [' + this.xmlHttpReqObject.oRequest.onreadystatechange + ']');
						this.xmlHttpReqObject.oGateway = this;
						this.xmlHttpReqObject.oRequest.open("GET", url, true);
						if (this.browser_is_ie) {
							this.xmlHttpReqObject.oRequest.send(null);
						} else {
							this.xmlHttpReqObject.oRequest.send();
						}
					}
				break;
			}
			...
			bool = false;
			_alertError(jsErrorExplainer(e, '(initXmlHttpRequest) :: initXmlHttpRequest threw an error.'));
		}
		return bool;
	},
	_transmitPacket : function() {  
	_alert('_transmitPacket()');
		this.counter++;  
		this.delayID = null; 
		this.received = null;
	...
	_alert('this.isXmlHttpPreferred = [' + this.isXmlHttpPreferred + ']');
		switch (this.method) {
			case this.Post_symbol:
