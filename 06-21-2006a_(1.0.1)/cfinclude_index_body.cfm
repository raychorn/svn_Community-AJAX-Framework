...
<cfif 0>
	...
	<!--
		if ( (browser_is_ie != null) && (browser_is_ff != null) ) {
	//		_alert('browser_is_ie = [' + browser_is_ie + ']' + ', browser_is_ff = [' + browser_is_ff + ']'); 
		} else {
			alert('ERROR: Unable to determine the browser type...');
		}
		...
		function handleGetCalendarMetaData(qObj) {
			...
			function calendarContent() {
				...
				if ( (_ISBIDI != null) && (_LOCALIZEDMONTH != null) && (_LOCALIZEDYEAR != null) && (_LOCALIZEDDAYS != null) && (_FIRSTWEEKPAD != null) && (_DAYCOUNTER != null) && (_DIM != null) && (_ROWCOUNTER != null) && (_DAYLIST != null) && (_FIRSTDAY != null) ) {
					...
					if (_ISBIDI) {
						html += '<button type="button" id="btn_calendar_prev" class="' + ((browser_is_ff) ? 'buttonMenuClassFF' : 'buttonClass') + '" onclick="performCalendarPrevMonth(\'' + aDict.asQueryString(',') + '\'); return false;">' + '&nbsp;&nbsp;&lt;&lt;&nbsp;&nbsp;' + '</button>';
						html += '<button type="button" id="btn_calendar_this" class="' + ((browser_is_ff) ? 'buttonMenuClassFF' : 'buttonClass') + '" onclick="performViewingArticlesByMonth(\'' + aDict.asQueryString(',') + '\'); return false;">' + '&nbsp;&nbsp;' + _LOCALIZEDMONTH + '&nbsp;' + _LOCALIZEDYEAR + '&nbsp;&nbsp;' + '</button>';
						html += '<button type="button" id="btn_calendar_next" class="' + ((browser_is_ff) ? 'buttonMenuClassFF' : 'buttonClass') + '" onclick="performCalendarNextMonth(\'' + aDict.asQueryString(',') + '\'); return false;">' + '&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;' + '</button>';
					} else {
						html += '<button type="button" id="btn_calendar_prev" class="' + ((browser_is_ff) ? 'buttonMenuClassFF' : 'buttonClass') + '" onclick="performCalendarPrevMonth(\'' + aDict.asQueryString(',') + '\'); return false;">' + '&nbsp;&nbsp;&lt;&lt;&nbsp;&nbsp;' + '</button>';
						html += '<button type="button" id="btn_calendar_this" class="' + ((browser_is_ff) ? 'buttonMenuClassFF' : 'buttonClass') + '" onclick="performViewingArticlesByMonth(\'' + aDict.asQueryString(',') + '\'); return false;">' + '&nbsp;&nbsp;' + _LOCALIZEDMONTH + '&nbsp;' + _LOCALIZEDYEAR + '&nbsp;&nbsp;' + '</button>';
						html += '<button type="button" id="btn_calendar_next" class="' + ((browser_is_ff) ? 'buttonMenuClassFF' : 'buttonClass') + '" onclick="performCalendarNextMonth(\'' + aDict.asQueryString(',') + '\'); return false;">' + '&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;' + '</button>';
					...
					for (i = _FIRSTWEEKPAD + 1; i <= 7; i++) {
						...
						html += '<td>' + ((_f > -1) ? '<button id="btn_calendarDay_' + (_FIRSTDAYdt.getMonth() + 1) + '_' + _DAYCOUNTER + '_' + _FIRSTDAYdt.getYear() + '" class="' + ((browser_is_ff) ? 'buttonMenuClassFF' : 'buttonClass') + '" onclick="performViewingArticlesByDay(this.id); return false;">' : '') + _DAYCOUNTER + ((_f > -1) ? '</button>' : '') + '</td>';
						_DAYCOUNTER++;
					}
					html += '</tr>';
					for (i = _DAYCOUNTER; i <= _DIM; i++) {
						...
						html += '<td>' + ((_f > -1) ? '<button id="btn_calendarDay_' + (_FIRSTDAYdt.getMonth() + 1) + '_' + i + '_' + _FIRSTDAYdt.getYear() + '" class="' + ((browser_is_ff) ? 'buttonMenuClassFF' : 'buttonClass') + '" onclick="performViewingArticlesByDay(this.id); return false;">' : '') + i + ((_f > -1) ? '</button>' : '') + '</td>';
						_ROWCOUNTER++;
						if (_ROWCOUNTER == 8) {
							html += '</tr>';
							_ROWCOUNTER = 1;
		...
		function handleGetBlogArchives(qObj) {
			...
			function searchForArchives(_ri, _dict, _rowCntName) {
				...
				if ( (!!_CATEGORYNAME) && (!!_CATEGORYID) && (!!_ENTRYCOUNT) ) {
					...
					html += '<tr>';
					html += '<td>';
					html += '<button type="button" id="btn_archives_' + _CATEGORYNAME + '" class="buttonDirectoryClass' + ((browser_is_ff) ? 'FF' : '') + '" onclick="performViewingArticlesByCategory(this, \'' + _CATEGORYID + '\'); return false;">' + '&nbsp;&nbsp;' + padStr + _ENTRYCOUNT + '&nbsp;&nbsp;' + ':' + '&nbsp;&nbsp;' + _CATEGORYNAME + '&nbsp;' + '</button>';
					html += '</td>';
					html += '</tr>';
				}
			};
		...
		function handleGetDefaultBlogArticles(qObj) {
			...
			function searchForBlogArticles(_ri, _dict, _rowCntName) {
				...
				if ( (_ALIAS != null) && (_ALLOWCOMMENTS != null) && (_BODY != null) && (_CATEGORYIDS != null) && (_CATEGORYNAMES != null) && (_COMMENTCOUNT != null) && (_ENCLOSURE != null) && (_FILESIZE != null) && (_ID != null) && (_MIMETYPE != null) && (_MOREBODY != null) && (_POSTED != null) && (_TITLE != null) && (_TRACKBACKCOUNT != null) && (_USERNAME != null) && (_POSTEDLONG != null) ) {
					...
					html += (((jsBool_sessionLoggedin) && (jsBool_isUserAdmin)) ? '&nbsp;|&nbsp;<button id="btn_adminDeleteEntry_' + _ID + '" class="buttonAdminDeleteEntryClass" onclick="performAdminDeleteEntry(this); return false;">Delete Entry</button>' : '');
					html += '</div>';
