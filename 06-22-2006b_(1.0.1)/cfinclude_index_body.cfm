...
<cfif 0>
	...
	//		_alert('document.body.scrollTop = [' + document.body.scrollTop + ']');
	//		_alert('windowOnscrollCallback(top = [' + top + '], left = [' + left + '])');
			var oTD = _$('td_loginRegisterFrame'); 
			var oAnchorPosTop = AnchorPosition.get$('anchor_articleContentTop');
...
// +++
	...
	//		html += '<span class="normalx12StatusBoldClass"><a href="" onclick="alert(\'More Info will be added soon.\'); return false;">More info &gt;&gt;</a></span>';
		...
		function populateAdminAddEntryGUI(btnObj, oDiv) {
			...
			html += '<td align="right" bgcolor="silver" width="*"><button id="btn_dismissAdminAddEntry" class="buttonClass" onclick="dismissAdminAddEntry(\'' + btnObj.id + '\', \'' + oDiv.id + '\'); return false;">[X]</button></td>';
			html += '</tr>';
			html += '<tr>';
			html += '<td colspan="2">';
			...
			html += '<td width="90%" align="left"><input type="text" name="title" value="" size="100" maxlength="100" class="textClass" style="width:100%"></td>';
			html += '</tr>';
			html += '<tr>';
			html += '<td colspan=2>'; // <b>' + js_entryBodyText + '</b><br>
			html += '<textarea name="body" cols=100 rows=10 class="textClass" style="width:100%"></textarea></td>';
			html += '</tr>';
			html += '<tr>';
			html += '<td width="10%" align="left"><b>' + js_postedatText + '</b></td>';
			html += '<td width="90%" align="left"><input type="text" name="posted" value="" class="textClass" style="width:100%"></td>';
			html += '</tr>';

			html += '<tr valign="top">';
			html += '<td width="10%" align="left"><b>' + js_categoriesText + '</b></td>';
			html += '<td width="90%" align="left">';
			html += '<select name="categories" multiple size=4 style="width:100%">';
			...
			html += '</td>';
			html += '<td width="90%" align="left">';
			html += '<input type="text" class="textClass" name="newcategory" value="" size="50" maxlength="50" style="width:100%">';
			html += '</td>';
			html += '</tr>';
	...
	//		html += '<tr>';
	//		html += '<td><b>#application.resourceBundle.getResource("alias")#</b></td>';
	//		html += '<td><input type="text" name="alias" value="#form.alias#" style="width:100%"></td>';
	//		html += '</tr>';
	//		html += '<tr>';
	//		html += '<td><b>#application.resourceBundle.getResource("allowcomments")#</b></td>';
	//		html += '<td>';
	//		html += '<select name="allowcomments">';
	//		html += '<option value="true" <cfif form.allowcomments is "true">selected</cfif>>Yes</option>';
	//		html += '<option value="false" <cfif form.allowcomments is "false">selected</cfif>>No</option>';
	//		html += '</select>';
	//		html += '</td>';
	//		html += '</tr>';
	//		html += '<tr valign="top">';
	...
	//		html += '<tr>';
	//		html += '<td>&nbsp;</td>';
	//		html += '<td><input type="submit" name="save" <cfif isDebugMode()>title="_url = [#_url#]"</cfif> value="#application.resourceBundle.getResource("save")#">';
	//		html += '<cfif url.id is not "new"><input type="submit" name="delete" value="#application.resourceBundle.getResource("delete")#" onClick="return confirm('#application.resourceBundle.getResource("areyousure")#')"></cfif> ';
	//		html += '<input type="submit" name="cancel" value="#application.resourceBundle.getResource("cancel")#" onClick="parent.launchBlogEditor(false <cfif url.id is not "new">, '#url.id#'</cfif>);"></td>';
	//		html += '</tr>';
			</cfif>
		...
		function handleGetDefaultBlogArticles(qObj) {
			...
			function searchForBlogArticles(_ri, _dict, _rowCntName) {
				...
				if ( (_ALIAS != null) && (_ALLOWCOMMENTS != null) && (_BODY != null) && (_CATEGORYIDS != null) && (_CATEGORYNAMES != null) && (_COMMENTCOUNT != null) && (_ENCLOSURE != null) && (_FILESIZE != null) && (_ID != null) && (_MIMETYPE != null) && (_MOREBODY != null) && (_POSTED != null) && (_TITLE != null) && (_TRACKBACKCOUNT != null) && (_USERNAME != null) && (_POSTEDLONG != null) ) {
					...
					html += (((jsBool_sessionLoggedin) && (jsBool_isUserAdmin)) ? '&nbsp;|&nbsp;<button id="btn_adminDeleteEntry_' + _ID + '" class="buttonAdminFunctionClass" onclick="performAdminDeleteEntry(this); return false;">Delete Entry</button>' : '');
					html += '</div>';
						...
						if (!bool_isAnyErrorRecords) {
							...
							if (!!dObj) {
								...
								if ((jsBool_sessionLoggedin) && (jsBool_isUserAdmin)) {
									...
									_html += '<button id="btn_adminAddEntry" class="buttonAdminFunctionClass" onclick="performAdminAddEntry(this); return false;">Add Entry</button>';
									// &nbsp;|&nbsp;
