	...
	<cfif (1)>
		...
		<cfif (Request.commonCode.isServerLocal())>
										...
										<cfif 0>
											...
											<button name="btn_registerButton" id="btn_registerButton" type="button" class="registerButtonClass3<cfif (Request.commonCode.isBrowserFF() OR Request.commonCode.isBrowserNS())>FF</cfif>" onclick="this.disabled = true; var oTD = _$('td_loginRegisterFrame'); var oFrame = _$('iframe_loginRegisterFrame'); if ( (!!oTD) && (!!oFrame) ) {  oTD.style.display = const_inline_style; oFrame.src = '#_url#'; }; return false;"><img src="#_urlImg#" height="20" border="0"></button>
										<cfelse>
											<button name="btn_registerButton" id="btn_registerButton" type="button" class="registerButtonClass<cfif (Request.commonCode.isBrowserFF() OR Request.commonCode.isBrowserNS())>FF</cfif>" onclick="this.disabled = true; performOpenLoginRegisterWindow(this.id, '#_url#'); return false;">Register</button>
										</cfif>
									</td>
		</cfif>
									<td>
										...
										<cfif (NOT bool_sessionLoggedin)>
											...
											<button name="btn_loginButton" id="btn_loginButton" type="button" class="loginButtonClass<cfif (Request.commonCode.isBrowserFF() OR Request.commonCode.isBrowserNS())>FF</cfif>" onclick="this.disabled = true; performOpenLoginRegisterWindow(this.id, '#_url#'); return false;">Login</button>
										<cfelse>
		...
		<cfif (NOT bool_sessionLoggedin)>
									<td>
										...
										<button name="btn_forgotPasswordButton" id="btn_forgotPasswordButton" type="button" class="forgotPasswordButtonClass<cfif (Request.commonCode.isBrowserFF() OR Request.commonCode.isBrowserNS())>FF</cfif>" onclick="this.disabled = true; performOpenLoginRegisterWindow(this.id, '#_url#'); return false;">Forgot Password</button>
									</td>
		</cfif>
		<cfif (bool_sessionLoggedin)>
									<td>
										...
										<button name="btn_purchasePremiumButton" id="btn_purchasePremiumButton" type="button" <cfif NOT (Session.persistData.blogname is 'default')>style="display: none;"</cfif> class="premiumButtonClass<cfif (Request.commonCode.isBrowserFF() OR Request.commonCode.isBrowserNS())>FF</cfif>" onclick="this.disabled = true; performOpenLoginRegisterWindow(this.id, '#_url#'); return false;">Premium Account</button>
									</td>
		</cfif>
	</cfif>
		<cfif (Request.commonCode.isServerLocal())>
