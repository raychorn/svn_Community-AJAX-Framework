							...
							if (NOT Request.dbError) {
								...
								if ( (NOT Request.dbError) AND (sid neq -1) ) {
									if (NOT bool_isServerNameTaken) {
										aFilePath = URL.userName & '+' & URL.serverName;
										isError = false;
										errorMsgHTML = '';
										try {
											...
											isError = true;
								//			errorMsgHTML = Request.commonCode.explainError(e, true);
										}
										if (NOT isError) {
											if ( (ListLen(URL.computerID, '|') eq 3) AND (Len(URL.serverName) gt 0) ) {
												...
												if (NOT Request.dbError) {
													...
													if (NOT Request.anError) {
														if ( (FindNoCase('.1.', CGI.SERVER_NAME) gt 0) OR (FindNoCase('.2.', CGI.SERVER_NAME) gt 0) ) {
															// copy the runtime license to the other server... if necessary...
															if (FindNoCase('.1.', CGI.SERVER_NAME) gt 0) {
																_url = 'http://' & Replace(CGI.SERVER_NAME, '.1.', '.2.') & ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/') & '/copyRegistration.cfm';
															} else if (FindNoCase('.2.', CGI.SERVER_NAME) gt 0) {
																_url = 'http://' & Replace(CGI.SERVER_NAME, '.2.', '.1.') & ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/') & '/copyRegistration.cfm';
															}
														} else {
															_url = 'http://' & CGI.SERVER_NAME & ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/') & '/copyRegistration.cfm';
														}
														ezCfHttp(_url, 'POST', 'serverName=' & URLEncodedFormat(aStruct.ServerName) & '&filePath=' & URLEncodedFormat(GetFileFromPath(aStruct.filePath)) & '&theLicense=' & URLEncodedFormat(aStruct._wddxDataStream));
														dataStream = 'OK';
													} else {
