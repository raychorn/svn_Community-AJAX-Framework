...
<cfif 0>
	...
	//	_alert('objectExplainer(window.location) = [' + objectExplainer(window.location) + ']');
		...
		function performOpenLoginRegisterWindow2(id) {
			...
			var oDiv = _$(((bool_isLoginForm) ? 'div_loginRegisterForm' : ((bool_isForgotForm) ? 'div_forgotPasswordForm' : ((bool_isRegisterForm) ? 'div_registerForm' : ''))));
			...
			function getRegisterLoginForgotFormContent(id) {
				...
				html += '<td><input type="text" id="' + ((bool_isForgotForm) ? 'input_forgot_username' : ((bool_isLoginForm) ? 'login_input_username' : 'register_input_username')) + '" size="50" maxlength="255"></td>';
				html += '</tr>';
				if (bool_isLoginForm) {
					html += '<tr>';
					...
					html += '<td><input type="text" id="register_input_yourname" value="" size="30" maxlength="50">&nbsp;<span class="redBoldPrompt">(Required - Your Name, first name and last name with a space between.)</span></td>';
					html += '</tr>';
			...
			//		alert('A. oDiv.innerHTML.length = [' + oDiv.innerHTML.length + ']');
					if (oDiv.innerHTML.length == 0) {
						var sHTML = (((bool_isLoginForm) || (bool_isForgotForm) || (bool_isRegisterForm)) ? getRegisterLoginForgotFormContent(id) : 'UNDEFINED OPERATION');
			//			_alert('B. sHTML = [' + sHTML + ']');
				...
				extraLoginRegisterHeightIncrement = 120 + ((bool_isRegisterForm) ? 50 : 0);
				extraLoginRegisterHeight += extraLoginRegisterHeightIncrement;
				repositionArticleContent();
			} else {
				alert('ERROR : Programming Error - Missing oTD [' + oTD + ']' + ', oDiv [' + oDiv + ']' + ', oODiv [' + oODiv + ']' + ', oBtn [' + oBtn + ']' + ', oBtn2 [' + oBtn2 + ']');
			}
		}
...
//	_alert('aDict = [' + aDict + ']')
	...
	//		html += '</tr>';
			...
			html += '<span class="normalx11StatusBoldClass"><a href="" onclick="alert(\'Downloads will be available soon.\'); return false;"><img border="0" src="' + ar2.join('/') + '"><br>Community Edition FREE Download</a></span>';
			html += '</td>';

			ar2[ar2.length - 1] = 'images/downloadTrialEdition.gif';

			html += '<td align="center" valign="top">';
			html += '<span class="normalx11StatusBoldClass"><a href="" onclick="alert(\'Downloads will be available soon.\'); return false;"><img border="0" src="' + ar2.join('/') + '"><br>Enterprise Trial Edition FREE Download</a></span>';
			html += '</td>';
...
// +++
	...
	//		html += '<span class="normalx12StatusBoldClass"><a href="" onclick="alert(\'More Info will be added soon.\'); return false;">More info &gt;&gt;</a></span>';
		...
		function handleGetBlogArchives(qObj) {
			...
			function searchForArchives(_ri, _dict, _rowCntName) {
				...
				if ( (!!_CATEGORYNAME) && (!!_CATEGORYID) && (!!_ENTRYCOUNT) ) {
					...
					html += '<tr>';
					html += '<td>';
					html += '<button type="button" id="btn_archives_' + _CATEGORYNAME + '" class="buttonDirectoryClass' + ((browser_is_ff || browser_is_ns) ? 'FF' : '') + '" onclick="performViewingArticlesByCategory(this, \'' + _CATEGORYID + '\'); return false;">' + '&nbsp;&nbsp;' + padStr + _ENTRYCOUNT + '&nbsp;&nbsp;' + ':' + '&nbsp;&nbsp;' + _CATEGORYNAME + '&nbsp;' + '</button>';
					html += '</td>';
					html += '</tr>';
				}
			};
		...
		function performAdminAddEntrySave() {
			...
			var oAdminAddEntryAllowComments = _$('select_adminAddEntryAllowComments');
			if ( (!!oAdminAddEntryTitle) && (oAdminAddEntryTitle.value) && (!!oAdminAddEntryBody) && (oAdminAddEntryBody.value) && (!!oAdminAddEntryPosted) && (!!oAdminAddEntryCategories) && (!!oAdminAddEntryNewCategory) && (!!oAdminAddEntryAlias) && (!!oAdminAddEntryAllowComments) ) {
				alert(123.4);
			} else {
		...
		function populateAdminAddEntryGUI(btnObj, oDiv) {
			var html = '';
			...
			function searchForArchives(_ri, _dict, _rowCntName) {
				...
				var _ENTRYCOUNT = _dict.getValueFor('ENTRYCOUNT');
				if ( (!!_CATEGORYNAME) && (!!_CATEGORYID) && (!!_ENTRYCOUNT) ) {
					html += '<option value="' + _CATEGORYID + '">' + _CATEGORYNAME + '</option>';
				}
			};
			// +++
			html += '<table width="98%" cellpadding="-1" cellspacing="-1">';
			html += '<tr>';
			html += '<td align="center" width="100%" bgcolor="silver"><span class="adminAddEntryClass">Add Entry</span></td>';
			...
			html += '<td width="90%" align="left"><input type="text" id="input_adminAddEntryTitle" value="" size="100" maxlength="100" class="textClass" style="width:100%"></td>';
			html += '</tr>';
			html += '<tr>';
			html += '<td colspan=2>'; // <b>' + js_entryBodyText + '</b><br>
			html += '<textarea id="ta_adminAddEntryBody" cols=100 rows=10 class="textClass" style="width:100%"></textarea></td>';
			html += '</tr>';
			html += '<tr>';
			html += '<td width="10%" align="left"><b>' + js_postedatText + '</b></td>';
			html += '<td width="90%" align="left"><input type="text" id="input_adminAddEntryPosted" value="" class="textClass" size="50" maxlength="50" style="width:100%"></td>';
			html += '</tr>';

			html += '<tr valign="top">';
			html += '<td width="10%" align="left"><b>' + js_categoriesText + '</b></td>';
			html += '<td width="90%" align="left">';
			html += '<select id="select_adminAddEntryCategories" multiple size=4 style="width:100%">';
			archivesByCategoryQObj.iterateRecObjs(searchForArchives);
			html += '</select>';
			html += '</td>';
			html += '</tr>';
			...
			html += '<td><input type="text" class="textClass" id="input_adminAddEntryAlias" value="" size="100" maxlength="100" style="width:100%"></td>';
			html += '</tr>';
			html += '<tr>';
			html += '<td><b>' + js_allowcommentsText + '</b></td>';
			html += '<td>';
			html += '<select id="select_adminAddEntryAllowComments">';
	...
	//		html += '<input type="submit" name="delete" value="#application.resourceBundle.getResource("delete")#" onClick="return confirm('#application.resourceBundle.getResource("areyousure")#')"> ';
	...
	//		_alert('archivesByCategoryQObj = [' + archivesByCategoryQObj + ']');

			<cfif 0>
	//		html += '<tr valign="top">';
	//		html += '<td><b>#application.resourceBundle.getResource("enclosure")#</b></td>';
	//		html += '<td>';
	//		html += '<input type="hidden" name="oldenclosure" value="#form.oldenclosure#">';
	//		html += '<input type="hidden" name="oldfilesize" value="#form.oldfilesize#">';
	//		html += '<input type="hidden" name="oldmimetype" value="#form.oldmimetype#">';
	//		html += '<cfif len(form.oldenclosure)>#listLast(form.oldenclosure,"/\")# <input type="submit" name="delete_enclosure" value="#application.resourceBundle.getResource("deleteenclosure")#"></cfif>';
	//		html += '<input type="file" name="enclosure" style="width:100%">';
	//		html += '<cfif (IsDefined("url.id"))>';
	//		html += '<input type="hidden" name="_id" value="#url.id#">';
	//		html += '</cfif>';
	//		html += '<cfif (IsDefined("Session.sessID"))>';
	//		html += '<input type="hidden" name="sessID" value="#Session.sessID#">';
	//		html += '</cfif>';
	//		html += '</td>';
	//		html += '</tr>';
			</cfif>
		...
		function performAdminAddEntry(oObj) {
			var oDiv = _$('div_adminAddEntryContent');
			if ( (!!oObj) && (!!oObj.id) && (!!oDiv) ) {
				oObj.disabled = true;
				oDiv.innerHTML = populateAdminAddEntryGUI(oObj, oDiv);
				// pop-up a window through which we add the content... reuse existing code where possible...
			} else {
