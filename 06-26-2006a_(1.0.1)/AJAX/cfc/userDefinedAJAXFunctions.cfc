<cfcomponent displayname="userDefinedAJAXFunctions" output="No" extends="ajaxCode">
	...
	<cfscript>
			...
			if ( (IsStruct(params)) AND (IsStruct(qryStruct)) ) {
				...
				if ( (maxEntries gt -1) AND (Request.articles.recordCount gt maxEntries) ) {
					...
					if (NOT Request.dbError) {
						...
						registerQueryFromAJAX(Request.articles2); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					} else {
					...
					registerQueryFromAJAX(Request.articles); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
				}
		...
		function getBlogArticleComments(id) {
			...
			registerQueryFromAJAX(Request.comments); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
		}
		...
		function getBlogArticleTrackbacks(id) {
			...
			registerQueryFromAJAX(Request.trackbacks); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
		}

		function userDefinedAJAXFunctions(qryStruct) {
			if (IsDefined("qryStruct.sessid")) {
			//	Client.sessID = qryStruct.sessid; // force the proper session ID for the ezAJAX(tm) Server...
			...
			switch (qryStruct.cfm) {
				case 'performCommitBlogCategory':
					if ( (IsDefined("qryStruct.namedArgs.catID")) AND ( (IsValid('UUID', qryStruct.namedArgs.catID)) OR (qryStruct.namedArgs.catID is '-1') ) AND (IsDefined("qryStruct.namedArgs.catName")) ) {
						...
						try {
							...
							isError = true;
							sErrorMsg = explainError(e, false);
						}
						if (NOT isError) {
						...
						QuerySetCell(qObj, 'errorMsg', 'Missing ezAJAX(tm) parm(s) named (#errorDetails#) for #qryStruct.cfm# in #CGI.SCRIPT_NAME#.', qObj.recordCount);
						registerQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					}
				break;
				...
				case 'performAdminAddEntry':
					...
					//  AND (IsDefined("qryStruct.namedArgs.postedAt")) AND (IsDefined("qryStruct.namedArgs.categories")) AND (IsDefined("qryStruct.namedArgs.newCategory")) AND (IsDefined("qryStruct.namedArgs.alias"))
					if ( (IsDefined("qryStruct.namedArgs.title")) AND (IsDefined("qryStruct.namedArgs.body")) AND (IsDefined("qryStruct.namedArgs.allowComments")) ) {
						...
						try {
							...
							_db = _db & 'C. errorStr = [#errorStr#]' & Chr(13);
							...
							_db = _db & 'E. errorStr = [#errorStr#] (#isDefined("qryStruct.namedArgs.categories")#), (#Len(Trim(qryStruct.namedArgs.categories))#), (#Len(Trim(qryStruct.namedArgs.newCategory))#)' & Chr(13);
							...
							_db = _db & 'F. errorStr = [#errorStr#]' & Chr(13);
							...
							if (Request.isAssignCategoriesError) {
								_db = _db & 'F.1 Request.isAssignCategoriesError = [#Request.isAssignCategoriesError#] (#Request.AssignCategoriesErrorMsg#)' & Chr(13);
								errorStr = errorStr & ', ' & Request.AssignCategoriesErrorMsg;
							}

							_db = _db & 'G. errorStr = [#errorStr#]' & Chr(13);
							...
							if (Len(errorStr) eq 0) {
								...
								if (Len(Trim(qryStruct.namedArgs.newCategory)) gt 0) {
									...
									_db = _db & 'G.1 newCatID = [#newCatID#], qryStruct.namedArgs.categories = [#qryStruct.namedArgs.categories#]' & Chr(13);
								}
							...
							_db = _db & 'H. errorStr = [#errorStr#]' & Chr(13);
							...
							id = -1;
							isError = true;
							sErrorMsg = errorStr & ', ' & explainError(e, false);

							_db = _db & 'I. sErrorMsg = [#sErrorMsg#]' & Chr(13);
						}

						_db = _db & 'J. isError = [#isError#], sErrorMsg = [#sErrorMsg#]' & Chr(13);

						if ( (NOT isError) AND (Len(errorStr) eq 0) ) {
							qObj = QueryNew('ID, entryID, title, body, postedAt, categories, newCategory, alias, allowComments, debug');
							QueryAddRow(qObj, 1);
							QuerySetCell(qObj, 'ID', qObj.recordCount, qObj.recordCount);
							...
							QuerySetCell(qObj, 'debug', _db, qObj.recordCount);
							registerQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
						} else {
							qObj = QueryNew('id, errorMsg, debug');
							QueryAddRow(qObj, 1);
							QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
						...
						if (NOT Request.dbError) {
							if (IsDefined("Session.persistdata.QAUTHUSER")) {
								...
								if (NOT Request.dbError) {
									...
									registerQueryFromAJAX(Request.QAUTHUSER); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
								} else {
									qObj = QueryNew('ID, ISVALID, PASSWORD, PREMIUMDATE, RENEWALFEE, UNAME, UROLE, USERNAME, LOGGEDIN');
									QueryAddRow(qObj, 1);
									QuerySetCell(qObj, 'ID', qObj.recordCount, qObj.recordCount);
					...
					if ( (IsDefined("qryStruct.namedArgs.username")) AND (IsDefined("qryStruct.namedArgs.yourname")) AND (IsDefined("qryStruct.namedArgs.password")) AND (IsDefined("qryStruct.namedArgs.confirmpassword")) ) { 
						if (qryStruct.namedArgs.password eq qryStruct.namedArgs.confirmpassword) {
							...
							if (NOT Request.dbError) {
								...
								QueryAddRow(qObj, 1);
								QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
								QuerySetCell(qObj, 'statusMsg', 'Your Registration was successful.\nYou will receive an email from us shortly with additional instructions you must follow to get your new User Account Activated.\nMake sure you upgrade your User Account to a Premium Account so you can take full advantage of all the feature this site has to offer you.', qObj.recordCount);
								registerQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
							} else {
								qObj = QueryNew('id, errorMsg, moreErrorMsg, explainError, isPKviolation');
								QueryAddRow(qObj, 1);
				...
				case 'getDirectoryContents':
					if ( (IsDefined("qryStruct.namedArgs.levelNum")) AND (IsDefined("qryStruct.namedArgs.buttonID")) AND (IsDefined("qryStruct.namedArgs.folderName")) ) { 
						...
						if ( (NOT Request.directoryError) AND (IsQuery(Request.qDir)) ) {
							registerQueryFromAJAX(Request.qDir); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
						...
						QuerySetCell(qObj, 'errorMsg', 'Missing ezAJAX(tm) parm named (#errorDetails#) for #qryStruct.cfm# in #CGI.SCRIPT_NAME#.', qObj.recordCount);
						registerQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					}
				break;
				...
				case 'performGetArticleByEntry':
					if (IsDefined("qryStruct.namedArgs.id")) {
						...
						QueryAddRow(qObj, 1);
						QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
						QuerySetCell(qObj, 'errorMsg', 'Missing ezAJAX(tm) parm named (id) for #qryStruct.cfm# in #CGI.SCRIPT_NAME#.', qObj.recordCount);
						registerQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					}
				break;
				...
				case 'performSubscribePodAction':
					if ( (IsDefined("qryStruct.namedArgs.emailAddress")) AND (Len(qryStruct.namedArgs.emailAddress) gt 0) ) {
						...
						QueryAddRow(qObj, 1);
						QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
						QuerySetCell(qObj, 'errorMsg', 'Missing ezAJAX(tm) parm named (emailAddress) for #qryStruct.cfm# in #CGI.SCRIPT_NAME#.', qObj.recordCount);
						registerQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					}
				break;
				...
				case 'getDefaultBlogArticles':
					if (IsDefined("qryStruct.namedArgs.maxEntries")) {
						...
						QueryAddRow(qObj, 1);
						QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
						QuerySetCell(qObj, 'errorMsg', 'Missing ezAJAX(tm) parm named (maxEntries) for #qryStruct.cfm# in #CGI.SCRIPT_NAME#.', qObj.recordCount);
						registerQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					}
				break;
				...
				case 'getBlogArchives':
					...
					if (NOT Request.dbError) {
						registerQueryFromAJAX(Request.qGetMaxEntryCount); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					} else {
						qObj = QueryNew('id, errorMsg, moreErrorMsg, explainError, isPKviolation');
						QueryAddRow(qObj, 1);
						QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
				...
				case 'getDownloadsFolder':
					if (IsDefined("qryStruct.namedArgs.folderName")) {
						...
						QueryAddRow(qObj, 1);
						QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
						QuerySetCell(qObj, 'errorMsg', 'Missing ezAJAX(tm) parm named (folderName) for #qryStruct.cfm# in #CGI.SCRIPT_NAME#.', qObj.recordCount);
						registerQueryFromAJAX(qObj); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					}
				break;
				...
				case 'getRecentArticles':
					...
					if (NOT Request.dbError) {
						...
						registerQueryFromAJAX(Request.entries2); // this function is used to tell the ezAJAX(tm) system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
					} else {
						qObj = QueryNew('id, errorMsg, moreErrorMsg, explainError, isPKviolation');
						QueryAddRow(qObj, 1);
						QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
