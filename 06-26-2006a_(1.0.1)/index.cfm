...
<cfif (NOT debugJavaScriptPackager)>
	...
	//		oAJAXEngine.isXmlHttpPreferred = (((window.location.href.toUpperCase().indexOf('LAPTOP.HALSMALLTALKER.COM') > -1) || (js_isDebugMode) || (oAJAXEngine.browser_is_ff)) ? false : true);
	//		oAJAXEngine.isXmlHttpPreferred = ((window.location.href.toUpperCase().indexOf('.CONTENTOPIA.NET') > -1) ? false : true);
		...
		// place this on the page where you want the gateway to appear
			oAJAXEngine.create();
			_alert('oAJAXEngine.myHTML = [' + oAJAXEngine.myHTML + ']');
		//-->
		</script>
