		...
		function userDefinedAJAXFunctions(qryStruct) {
			...
			switch (qryStruct.cfm) {
				case 'performAdminAddEntry':
					writeOutput(cf_dump(qryStruct.namedArgs, 'qryStruct.namedArgs', false));
					if (IsStruct(qryStruct.namedArgs)) {
						keysAR = StructKeyArray(qryStruct.namedArgs);
						for (i = 1; i lte ArrayLen(keysAR); i = i + 1) {
							qryStruct.namedArgs[keysAR[i]] = URLDecode(qryStruct.namedArgs[keysAR[i]]);
						}
					}
					...
					if ( (IsDefined("qryStruct.namedArgs.title")) AND (IsDefined("qryStruct.namedArgs.body")) AND (IsDefined("qryStruct.namedArgs.allowComments")) ) {
						...
						try {
							...
							if ( (NOT isDefined("qryStruct.namedArgs.categories")) OR (Len(Trim(qryStruct.namedArgs.categories)) eq 0) AND (Len(Trim(qryStruct.namedArgs.newCategory))) ) {
							...
							if (Len(errorStr) eq 0) {
								...
								if (Len(Trim(qryStruct.namedArgs.newCategory)) gt 0) {
									...
									qryStruct.namedArgs.categories = ListAppend(qryStruct.namedArgs.categories, application.blog.addCategory(qryStruct.namedArgs.newCategory), ',');
								}
